    ...
    public class CalculatorDisplayUtility: ICalculatorDisplayUtility
    {
        ...
        private bool handleUserInput(ICalculator calculator)
        {
            ...
            switch (option)
            {
                ...
                case 3:
                case 4:
                    var amountOfOperands = receiveOperandAmount();
                    if (string.IsNullOrEmpty(amountOfOperands))
                    {
                        amountOfOperands = "0";
                    }

                    calculator.Operands = new double[Convert.ToInt32(amountOfOperands)];

                    for (int index = 0; index < calculator.Operands.Length; index++)
                    {
                        calculator.Operands[index] = receiveUserInputForOperand();
                    }
